...
Don't forget that you'll need to install those packages with `install.packages()` if you haven't already done so.
...
p2 <- ggplot(diamonds, aes(x = log10(carat), y = log10(price))) + geom_point()
...
Useful alternatives are the `RColorBrewer` scales which we have seen before and have been hand tuned to work better for people with common types of color blindness.
<<<<<<< HEAD
=======

>>>>>>> 6289f4d (work through)
